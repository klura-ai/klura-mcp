...
module.exports = function defineTools(klura) {
  ...
  return [
    {
      name: 'start_session',
      description: `Start a klura session: open a browser and navigate to the given URL. Returns \`{sessionId, a11yTree, url, artifacts?, executed?, execute_result?, graph?}\`. The \`graph\` parameter selects one of: ${graphModes.map((g) => `"${g}"`).join(', ')}. "discover" (default) — drive→triage→lift→closed, the standard goal-directed reverse-engineering flow; "map" — drive→closed, surface-mapping with mutating-action consent gates and skipped auto-synth; "execute" — execute→triage→lift→closed (or terminal{failed}), runs a saved strategy and falls into triage on stale-strategy failure so the agent can re-plan and re-lift. When you pass \`{capability, args}\` and a complete saved strategy covers that capability, the runtime auto-runs the strategy in-session and returns \`executed: true\` with the result — call end_drive and you are done.`,
      inputSchema: {
        type: 'object',
        properties: {
          ...
          capability: { type: 'string', description: 'The capability slug being discovered or executed (e.g. "send_message"). Required for auto-execute and for auto-save at end_drive.' },
          args: { type: 'object', description: 'Per-capability argument map: {paramName: literalValue}. These are the user-supplied values the agent will type (e.g. {text: "hello", recipient: "Bob"}). Used at auto-execute time to run the saved strategy, and at end_drive time to template captured traffic into a reusable strategy body.' },
          policy: {
            type: 'object',
...

    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          ws_hash: { type: 'string', description: 'Content-addressed handle for the frame (returned on every shaped frame and on the end_drive RE nag). Survives ring rotation; only way to address a pinned frame. Use instead of ws_i when available.' },
...

    {
      ...
      handler: (args) => klura.endDrive(args.session_id, {
        platform: args.platform,
...

    {
      ...
      description: 'Remove an active breakpoint by id. Idempotent — removing an unknown/already-removed id is a no-op. Use when you are done with a bp, or call end_drive and the runtime will clean up automatically.',
      inputSchema: {
        type: 'object',
        properties: {
          session_id: { type: 'string' },
    ...
    {
      name: 'resume',
      description: 'Release the current pause and let the page continue. No-op when the session isn\'t paused. Use after you have extracted everything you need from the paused frame. end_drive also auto-resumes, so in practice this tool is optional if you are about to close.',
      inputSchema: {
        type: 'object',
        properties: { session_id: { type: 'string' } },
        required: ['session_id'],
